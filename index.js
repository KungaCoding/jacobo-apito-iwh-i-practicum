...
const CUSTOM_OBJECT_URL = 'https://api.hubapi.com/crm/v3/objects/2-230139739';
const headers = {
...
app.get('/', async (req, res) => {
    try {
        const resp = await axios.get(CUSTOM_OBJECT_URL, {
            headers,
            params: {
                properties: 'name,element,power_level'
            }
        });
...
app.post('/update-cobj', async (req, res) => {
    ...
    try {
        await axios.post(CUSTOM_OBJECT_URL, newRecord, { headers });
        res.redirect('/');
    } catch (error) {
        console.error(error.response?.data || error.message);
